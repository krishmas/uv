...
fn tool_install_no_editable_workspace_member() -> Result<()> {
    ...
    child_src.child("__init__.py").touch()?;

    // Install the tool from the workspace root with --no-editable.
    uv_snapshot!(context.filters(), context.tool_install()
        .arg("--no-editable")
    ...
    // Remove the source for the child package to simulate a broken editable link.
    fs_err::remove_dir_all(&child)?;

    // The installed tool should still work if child was installed non-editable.
    uv_snapshot!(context.filters(), Command::new("root_cli").env(EnvVars::PATH, bin_dir.as_os_str()), @r"
    success: true
    exit_code: 0
    ----- stdout -----
